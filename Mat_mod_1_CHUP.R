...
Bashkortostan = data.frame(id = "Bashkortostan", latitude = 54.78517, longitude = 56.04562)
Bashkortostan
#найдем станции, соответствующие критериям
Bashkortostan_around = meteo_nearby_stations(lat_lon_df = Bashkortostan, station_data = station_data,
                                             limit = 10, var = c("PRCP", "TAVG"),
                                             year_min = 1990, year_max = 2005)
Bashkortostan_around
# Определим список станций
Bashkortostan_id = Bashkortostan_around[["Bashkortostan"]][["id"]]

summary(Bashkortostan_id)
str(Bashkortostan_id)

# попробуем для 2 метеостанции
Bashkortostan_id [3]
# 
dataAA = meteo_tidy_ghcnd(stationid = Bashkortostan_id [3],
                          var="TAVG",
                          date_min="1990-01-01",
                          date_max="2005-12-31")
dataAA
...
#создадим таблицу 
all_data = tibble()
for (i in 1:length(Bashkortostan_around$Bashkortostan[,1]))
  
{
  
  print(i)
  print(Bashkortostan_id)
  # Загрузим данные для станции:
  
  data = meteo_tidy_ghcnd(stationid = Bashkortostan_id[i],
                          var="TAVG",
                          date_min="1990-01-01",
                          date_max="2005-12-31")
  print(data)
...
write.csv(all_data, "all_Bashkortostan_data.csv")
...
#Согласно расчету, урожайность пшеницы в период с 1990 по 2005 год в Башкортостане составила (ц/га):
